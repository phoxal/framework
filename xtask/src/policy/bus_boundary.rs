...
/// directory, or a unit-test module split out into its own `*_tests.rs` beside
/// the code it proves.
///
...
fn is_test_file(path: &str) -> bool {
    path.split('/').any(|segment| segment == "tests") || path.ends_with("_tests.rs")
}
...
mod tests {
    ...
    /// The two whole-file shapes of test code, and the production modules whose
    /// names only look like them.
    #[test]
