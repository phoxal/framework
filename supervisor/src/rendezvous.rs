...
mod tests {
    ...
    fn source_and_installed_roots_map_to_the_shared_contract() {
        ...
        assert_eq!(source.ownership_root.as_path(), Path::new("/work/robot"));
        assert_eq!(
            source.volatile_root.as_path(),
            Path::new("/work/robot/.phoxal/run")
        );
        assert_eq!(
        ...
        for installed in [
            ...
            assert_eq!(
                paths.ownership_root.as_path(),
                Path::new(ACTIVE_RUNTIME_ROOT)
            );
            assert_eq!(
                paths.volatile_root.as_path(),
