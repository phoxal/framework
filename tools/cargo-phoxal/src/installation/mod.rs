...
// The installation module is dormant: it was relocated into cargo-phoxal
// because the former phoxal-installation package no longer justifies a
// separate Cargo crate, but no production command in the tool currently
// drives it. Until the install/deploy subcommand is added, suppress the
// unused-code warnings the dormant surface would otherwise raise. The
// public surface and tests are kept verbatim so the first production
// caller can wire it back in without a rewrite.
