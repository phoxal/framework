...
#[phoxal::driver(state = Bno085State, api = Api, connection = i2c)]
...
mod tests {
    ...
    async fn setup_fails_before_publishing_when_hardware_backend_is_unavailable() {
        let bus = phoxal::testing::TestBus::for_participant("bno085-test")
            .await
            .expect("the in-process test bus opens");
        let launch =
            phoxal::testing::TestHarness::new("bno085-test").expect("valid test participant");
        let result = phoxal::testing::run_test_harness::<Bno085, _>(
            bus.handle(),
            launch,
            std::future::pending(),
        )
