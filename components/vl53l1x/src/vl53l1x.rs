...
#[phoxal::driver(state = Vl53l1xState, api = Api, connection = i2c)]
...
mod tests {
    ...
    async fn setup_fails_before_publishing_when_hardware_backend_is_unavailable() {
        let bus = phoxal::testing::TestBus::for_participant("vl53l1x-test")
            .await
            .expect("the in-process test bus opens");
        let launch =
            phoxal::testing::TestHarness::new("vl53l1x-test").expect("valid test participant");
        let result = phoxal::testing::run_test_harness::<Vl53l1x, _>(
            bus.handle(),
            launch,
            std::future::pending(),
        )
