...
Phoxal gives a robot a small, strongly-typed core: a contract bus over Zenoh, generation-qualified contracts, and a derive-based participant authoring model.
...
phoxal-api = "0.20"  # the contract tree: `use phoxal_api::y2026_1 as api;`
```
...
- Import the dated API modules you need, such as `use phoxal_api::y2026_1 as api;` and `use phoxal_api::y2026_7;`.
  An `Api` struct may mix contract bodies from different generations field by field, so there is no participant-wide API generation to declare.
...
- The wire body is the plain payload, and contract identity lives only in the generation-qualified topic key.
  Bus metadata carries the codec and provenance, including logical produce time and source identity, but no API generation or contract family.
...
| [`phoxal::bus`] | Re-export of the `phoxal-bus` crate: the key scheme `<namespace>/robots/<robot-id>/<generation-qualified-topic>`, the named-field MessagePack codec, provenance-only `BusMetadata`, and the body-typed handles `Publisher`/`Subscriber`/`Latest`/`Querier`. |
