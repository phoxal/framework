...
/// The framework-owned authority policy for the built-in drive service.
///
/// Native and physical motor implementations consume this policy. It is not a
/// simulator concern and it only creates leases for the motor command payload
/// the drive service actually owns.
#[derive(Clone, Debug)]
...
impl DriveCommandAuthority {
    ...
    /// Create the fail-closed lease for one typed motor command lane.
    #[must_use]
...
pub struct Target {
    #[serde(deserialize_with = "crate::api::drive::deserialize_finite_target_scalar")]
