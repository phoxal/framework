...
/// The dated API contract tree, re-exported from the `phoxal-api` crate (the
/// version-local wire bodies + topic builders generated by
/// [`phoxal_api_tree!`](macro@phoxal_macros::phoxal_api_tree), over the
/// `phoxal-bus` ABI floor). Stays addressable at `phoxal::api::*` - so
/// `phoxal::api::y2026_1`, `phoxal::api::ApiVersion`, and
/// `phoxal::api::ContractBody` keep resolving, and the engine macros' `::phoxal::api::…`
/// output resolves through here.
///
/// This is a facade *module* (`pub mod api { pub use phoxal_api::*; }`), not a
/// crate rename (`pub use phoxal_api as api;`). rustc renders re-exported items
/// through the path the author actually has in scope only when the re-export is a
/// module here; a bare crate rename leaks the underlying crate name into
/// diagnostics (`phoxal::phoxal_api::…`). Keeping `api` a real module makes
/// compiler errors read `phoxal::api::y2026_1::…`, matching the authoring surface.
pub mod api {
    pub use phoxal_api::*;
}
