...
impl Program {
    ...
    pub fn normalize(
        ...
        let program_bytes = serde_json::to_vec(&envelope).map_err(|error| ProgramError::Other(error.to_string()))?;
    ...
    WireStep {
        ...
        action: match &step.action {
            Action::Setpoint {
                ...
                let (name, service, method, kind, request, response) = sig_strings(consumer_signature);
                WireAction::Setpoint {
            ...
            Action::Withdraw { producer_signature } => {
                let (name, service, method, kind, request, response) = sig_strings(producer_signature);
                WireAction::Withdraw {
            ...
            Action::Command {
                ...
                let (name, service, method, kind, request, response) = sig_strings(service_signature);
                WireAction::Command {
...
fn sig_strings(sig: &phoxal_port::PortSignature) -> (&'static str, &'static str, &'static str, String, &'static str, &'static str) {
    (
        sig.name,
        sig.service,
        sig.method,
...
fn sha256_hex(bytes: &[u8]) -> String {
    ...
    for chunk in msg.chunks_exact(64) {
        ...
        for i in 0..64 {
            ...
            let ch = (e & f) ^ (!e & g);
            let t1 = (Wrapping(hh) + Wrapping(s1) + Wrapping(ch) + Wrapping(K[i]) + Wrapping(w[i])).0;
