...
pub trait InputSet: 'static {
    ...
    /// The subset of fields with generated descriptors that can receive
    /// standard-Protobuf Runtime traffic.
    const TRANSPORT_FIELDS: &'static [super::transport::InputTransportField] = &[];

    /// Returns the generated descriptor-backed input fields.
    fn transport_fields() -> &'static [super::transport::InputTransportField] {
        Self::TRANSPORT_FIELDS
    }

    /// Admit one bounded transport batch into an immutable input cut.
    ///
    /// Implementations are generated by the input collector.  The default is
    /// deliberately an explicit refusal so a declared field can never be
    /// silently discarded by a transport runner.
...
/// Generated input bindings that can admit typed Runtime transport records.
///
/// `InputSet` deliberately remains usable for direct, transport-free runtime
/// tests.  This companion trait carries the stronger generated Prost bounds
/// required by the process runner, so private scalar fixtures do not acquire
/// a fake wire representation merely by using the input collector.
pub trait TransportInputSet: InputSnapshot {
    /// Every generated field that can be connected to a Runtime transport.
    const TRANSPORT_FIELDS: &'static [super::transport::InputTransportField];

    /// Returns the generated transport fields in declaration order.
    fn transport_fields() -> &'static [super::transport::InputTransportField] {
        <Self as TransportInputSet>::TRANSPORT_FIELDS
    }

    /// Admit one complete bounded transport batch into this input cut.
    fn decode_transport_field(
        &mut self,
        field: &str,
    ...
    fn decode_transport_field_with_keys(
        ...
        let _ = keys;
        TransportInputSet::decode_transport_field(self, field, binding, samples)
    }

    /// Admit a batch with the host-freeze time available to freshness-aware
    /// generated decoders.
...
/// A generated decoder implementation attached to one input snapshot type.
///
...
pub trait GeneratedTransportDecoder<Inputs>: 'static {
    ...
    /// Decode one complete bounded field batch.
    fn decode(
        inputs: &mut Inputs,
        field: &str,
        binding: Option<&super::transport::PortBinding>,
    ...
    fn decode_at(
        ...
        let _ = now;
        Self::decode(inputs, field, binding, samples)
    }

    /// Decode using managed activation key lookup when a process runner
    /// assigned transport correlations locally.
    fn decode_with_keys(
        inputs: &mut Inputs,
        field: &str,
        binding: Option<&super::transport::PortBinding>,
    ...
    /// Decode with managed keys and the host-freeze time available for
    /// freshness-aware generated decoders.
...
impl GeneratedTransportDecoder<()> for () {
    const FIELDS: &'static [super::transport::InputTransportField] = &[];
...
impl<T> TransportInputSet for T
where
    T: InputSnapshot,
    T::Transport: GeneratedTransportDecoder<T>,
{
    const TRANSPORT_FIELDS: &'static [super::transport::InputTransportField] =
        <T::Transport as GeneratedTransportDecoder<T>>::FIELDS;
    ...
    fn decode_transport_field_with_keys(
        ...
        <T::Transport as GeneratedTransportDecoder<T>>::decode_with_keys(
            self, field, binding, samples, keys,
        )
    }
    ...
    fn expire_transport_fields_at(&mut self, now: ExecutionTime) -> crate::Result<()> {
        <T::Transport as GeneratedTransportDecoder<T>>::expire_at(self, now)
    }
}
...
pub trait InputSnapshot: InputSet {
    /// Marker type carrying the generated transport decoder for this input
    /// snapshot.  `()` denotes a transport-free direct fixture.
...
/// A value decoded by a generated transport binding before it is installed in
/// the concrete input form.
pub type TransportValue = Box<dyn Any + Send>;

/// Resolves a locally assigned transport correlation back to its original
/// activation key.  The runner owns this table and consumes each key exactly
/// once when a typed Read/Request reply is admitted.
pub trait TransportKeyLookup {
    /// Remove and return the key associated with one generated input field
    /// and wire command identifier.
    fn take_key(&mut self, field: &str, command_id: u64) -> Option<TransportValue>;

    /// Validates the provider identity attached to a correlated reply before
    /// its generated response body is decoded.  A stale reply may be
    /// discarded by [`Self::is_expired`] without consulting this hook.
    fn validate_reply(
        &mut self,
        _field: &str,
        _command_id: u64,
        _sample: &super::transport::WireSample,
    ) -> crate::Result<()> {
        Ok(())
    }

    /// Reports whether a reply correlation was retired by its transfer
    /// deadline.  Retired replies are drained and discarded instead of
    /// turning a late network delivery into a runtime fault.
    fn is_expired(&mut self, _field: &str, _command_id: u64) -> bool {
        false
    }
}

/// One stamped value decoded from a publication transport record.
pub struct TransportSample {
    /// The erased generated Prost value.
    pub value: TransportValue,
    /// The source capture stamp carried by the record.
    pub stamp: ObservationStamp,
}

/// One stream record decoded from the transport boundary.
pub enum TransportStreamItem {
    /// A stamped generated Prost value.
    Data(TransportSample),
    /// The source reported a continuity gap.
    Gap,
    /// The source reached normal end of stream.
    End,
    /// The source reached a terminal failure.
    Failed(String),
}

/// One command decoded from the transport boundary while retaining its
...
///
/// Operation results never cross the Runtime port wire.  The output side
/// deposits this bounded record into the input side's next freeze so the
/// service observes it as an ordinary keyed input completion.
pub struct OperationCompletionRecord {
    /// The selected local operation input field.
    pub field: &'static str,
    /// The activation key retained without imposing a wire representation.
    pub key: TransportValue,
    /// The worker result or its supervised timeout/failure.
    pub result: Result<TransportValue, OperationInputError>,
}

/// Installs decoded generated values into one concrete input snapshot.
///
/// This erased hand-off keeps private scalar fixtures valid for direct
/// runtime tests while generated Prost payloads are checked by the transport
...
pub trait TransportInputSink {
    ...
    /// Install a bounded samples batch.
    fn set_samples(
        &mut self,
        field: &str,
        values: Vec<TransportSample>,
        gap: bool,
    ) -> crate::Result<()>;

    /// Install a bounded events batch.
    fn set_events(
        &mut self,
        field: &str,
        values: Vec<TransportValue>,
        gap: bool,
    ) -> crate::Result<()>;
    ...
    /// Install a bounded, ordered command batch.
    fn set_commands(
        &mut self,
        field: &str,
        values: Vec<TransportCommand>,
        encoded_bytes: u64,
    ) -> crate::Result<()>;

    /// Install a keyed read completion.
    fn set_read(
        &mut self,
        field: &str,
        key: TransportValue,
    ...
    /// Install a runner-owned local operation completion.
    fn set_operation(
        &mut self,
        field: &str,
        key: TransportValue,
...
impl TransportInputSink for () {
    fn set_latest(
        &mut self,
        field: &str,
        _value: TransportValue,
    ...
    fn set_stream(&mut self, field: &str, _values: Vec<TransportStreamItem>) -> crate::Result<()> {
        Err(anyhow::anyhow!(
            super::transport::TransportError::InvalidMetadata {
                detail: format!("input field `{field}` has no generated transport sink"),
            }
    ...
    fn set_request(
        ...
        field: &str,
        _key: TransportValue,
        _result: Result<TransportValue, RequestError>,
    ) -> crate::Result<()> {
        Err(anyhow::anyhow!(
            super::transport::TransportError::InvalidMetadata {
                detail: format!("input field `{field}` has no generated transport sink"),
    ...
    fn set_operation(
        ...
        field: &str,
        _key: TransportValue,
        _result: Result<TransportValue, OperationInputError>,
    ) -> crate::Result<()> {
        Err(anyhow::anyhow!(
            super::transport::TransportError::InvalidMetadata {
                detail: format!("input field `{field}` has no generated transport sink"),
...
impl<T> Setpoint<T> {
    ...
    /// Creates an admitted intent from its exact wire validity interval.
    ///
    /// The transport carries the expiry instant rather than a duration so a
...
impl<T> Stream<T> {
    ...
    /// Reports whether this batch contains an End or Failed terminal record.
    #[must_use]
    pub fn is_terminal(&self) -> bool {
        self.items
            .iter()
            .any(|item| matches!(item, StreamItem::End | StreamItem::Failed(_)))
    }
}
...
pub struct CommandOrder {
    ...
    /// `None` for an authored graph caller and `Some` for supervisor ingress.
    /// The field is ordered before the caller rank so controlled callers
    /// always precede external ingress at one eligible boundary.
    ingress_sequence: Option<u64>,
    caller_rank: u64,
    sequence: CommandId,
}
...
impl CommandOrder {
    ...
    /// Creates an order key for supervisor-owned external ingress.
    #[must_use]
    ...
    /// Returns the supervisor ingress sequence, when this is an external
    /// command.  Authored graph callers return `None`.
    #[must_use]
    pub const fn ingress_sequence(self) -> Option<u64> {
        self.ingress_sequence
    }

    /// Returns whether this order came from supervisor-owned external ingress.
    #[must_use]
...
impl<Response> Reply<Response> {
    ...
    /// Creates a reply while retaining the complete deterministic command
    /// order used when its request was admitted.
    #[must_use]
    pub fn with_order(order: CommandOrder, response: Response) -> Self {
        Self { order, response }
    }
    ...
    /// Returns the complete deterministic order of the originating command.
    #[must_use]
    pub const fn order(&self) -> CommandOrder {
        self.order
    }
...
mod tests {
    ...
    fn command_reply_preserves_queue_admission_identity() {
        let order = CommandOrder::new(12, 4, CommandId::new(7));
        let commands = Commands::new(vec![Command::<u8, u16>::with_order(order, 3)]);
