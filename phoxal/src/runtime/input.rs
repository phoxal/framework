...
/// Provides the exact generated request codec for a Read or Request
/// activation.  It is implemented only for Prost-backed payloads, and is
/// requested by generated activation methods rather than by every input
/// declaration.  This keeps private transport-free fixtures valid.
#[doc(hidden)]
pub trait ActivationRequestCodec {
    /// Returns the request encoder/decoder pair used by the runtime wire.
    fn request_codec() -> Option<crate::port::PortCodec>;
}

/// Calls a generated activation request codec through a type-directed helper.
#[doc(hidden)]
pub fn activation_request_codec<T: ActivationRequestCodec>() -> Option<crate::port::PortCodec> {
    T::request_codec()
}

impl<Key: 'static, Request: 'static, Response: 'static> ActivationRequestCodec
    for Read<Key, Request, Response>
{
    fn request_codec() -> Option<crate::port::PortCodec> {
        super::transport::registered_codec::<Request>()
    }
}

impl<Key: 'static, RequestBody: 'static, Response: 'static> ActivationRequestCodec
    for Request<Key, RequestBody, Response>
{
    fn request_codec() -> Option<crate::port::PortCodec> {
        super::transport::registered_codec::<RequestBody>()
    }
}
