...
/// Publish every complete attachment phase in supervisor revision order.
async fn serve_attachments(bus: BusHandle, state: ExecutionState) -> Result<()> {
    let publisher =
...
async fn serve_current_attachment(bus: BusHandle, state: ExecutionState) -> Result<()> {
    let server = declare(
        &bus,
        &supervisor::topics()
            .simulation()
            ...
            supervisor::simulation::SimulationAttachmentPhase::Preparing => {
                ...
                .await
                {
                    match state.abort_preparing_attachment(host, preparing.revision) {
                        ...
                        Err(super::state::AttachmentStateError::NotPreparing) => {}
...
async fn serve_attachment_liveness(
    bus: BusHandle,
    state: ExecutionState,
    shutdown: CancellationToken,
) -> Result<()> {
    ...
    loop {
        ...
        match state.fail_active_attachment(active.revision, reason) {
            ...
            Err(super::state::AttachmentStateError::NotActiveRevision) => continue,
...
async fn finish_clean_simulation_removal(bus: &BusHandle, state: &ExecutionState) -> Result<()> {
