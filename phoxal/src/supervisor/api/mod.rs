...
///
/// A payload module with no endpoints of its own: the projection is what
/// [`snapshot`] publishes and answers with, so it is declared here beside the
/// family rather than under a node nothing addresses.
pub mod execution;
