//! Bus-ABI golden bindings against the real `v1` API tree.
//!
...
//! `phoxal_api_tree!`-generated `ContractBody`/`ApiVersion` impls for `v1`
...
fn contract_body_topic_is_version_qualified_on_the_real_tree() {
    // D1: the version is folded into the wire key, so a real v1 contract
    // publishes on a key that cannot collide with a hypothetical v2
