...
pub(crate) fn expand_fragment(input: TokenStream) -> syn::Result<TokenStream> {
    ...
    Ok(quote! {
        #[doc(hidden)]
        pub(crate) struct __ProtocolFragmentMarker;
        #[doc(hidden)]
        pub(crate) trait __ProtocolFragmentRegistered { const REGISTERED: (); }
        const _: () = <__ProtocolFragmentMarker as __ProtocolFragmentRegistered>::REGISTERED;

        macro_rules! __protocol_fragment_collect {
        ...
        pub(crate) use __protocol_fragment_collect;
    })
}
...
pub(crate) fn expand_fragment_group(input: TokenStream) -> syn::Result<TokenStream> {
    ...
    let collectors: Vec<TokenStream> = group
        .paths
        .iter()
        .map(|path| quote! { #path::__protocol_fragment_collect })
        .collect();
    let registrations = group.paths.iter().map(|path| {
        quote! {
            impl #path::__ProtocolFragmentRegistered for #path::__ProtocolFragmentMarker {
                const REGISTERED: () = ();
            }
        }
    });
    ...
    Ok(quote! {
        ...
        macro_rules! __protocol_group_chain {
        ...
        #first!(
            __protocol_group_chain,
            ::phoxal_macros::__protocol_define_group,
            (#(#tail),*),
            ()
        );
    })
...
pub(crate) fn expand_tree(input: TokenStream) -> syn::Result<TokenStream> {
    ...
    Ok(quote! {
        ...
        macro_rules! __protocol_collect_chain {
            ($chain:path, $finish:path, (), ($($collected:tt)*)) => {
        ...
        #first!(
            __protocol_collect_chain,
            ::phoxal_macros::__protocol_materialize,
            (#(#tail),*),
            ()
        );
    })
