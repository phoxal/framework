...
/// identity, not the body type as written in source (every participant
/// writes `use phoxal_api::y2026_1 as api;`, so a source-text string like
/// `api::drive::Target` has the generation erased and cannot distinguish a
/// `y2026_1` contract from a same-named `y2026_7` one; `GENERATION`/`CONTRACT`
/// are generated by the same api-tree macro that writes the type there, so
/// they always carry the generation). Recording the two as separate JSON
/// fields (rather than one joined `NAME`) keeps every downstream reader
/// (xtask's coherence gate, the catalog) independent of the generation naming
...
pub fn expand_api(input: TokenStream) -> syn::Result<TokenStream> {
    ...
    // it has no bearing on the *value* recorded for `generation`/`contract`,
    // which are the resolved `GENERATION`/`CONTRACT` consts (built as tokens
    ...
    // The manifest JSON's `generation`/`contract` values are the RESOLVED
    // `GENERATION`/`CONTRACT` consts, not macro-time string literals, so the
...
/// `{"role":"...","generation":"...","contract":"...","external":...}`, a
/// nested `__concatcp!` call splicing the resolved `<Body as
/// ContractBody>::GENERATION`/`::CONTRACT` between macro-time-literal JSON
/// fragments. `role` and `external` are known at macro-expansion time (baked
/// in as literals); `generation`/`contract` are not - each is spliced in as a
