...
/// ContractBody>::VERSION`/`::CONTRACT`, e.g. `"v1"` /
...
/// a version module, so a source-text string like `api::drive::Target` has
/// the version erased and cannot distinguish a `v1` contract from a
/// same-named `v2` one; `VERSION`/`CONTRACT`
...
/// (xtask's coherence gate, the catalog) independent of the version naming
