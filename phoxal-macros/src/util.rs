...
/// pulls in as a dev-dependency for its own tests/examples). The `#[derive(Service)]`
/// and `#[derive(Driver)]` output names `phoxal_api::y2026_N::Api` to bind a
/// participant's one API version, since that marker type lives only here and is not
/// re-exported through the engine's `phoxal::bus` ABI floor.
