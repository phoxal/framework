...
pub fn expand_api(input: TokenStream) -> syn::Result<TokenStream> {
    let tree: ApiTree = syn::parse2(input)?;
    match tree {
        ApiTree::Api { versions, latest } => {
            for version in &versions {
                Node::reject_legacy_roles(&version.nodes)?;
                Node::reject_authored_types(&version.nodes)?;
            }
            ApiTree::expand_api(&versions, &latest, true)
        }
...
/// [`expand_api`] makes the protocol-mode concern explicit at the call site.
...
impl ApiTree {
    ...
    fn expand_api(
        ...
        let catalogue_variants = versions.iter().map(|version| {
            let variant = format_ident!("V{}", version.name.to_string().trim_start_matches('v'));
            let wire_id = &version.wire_id;
            let process_id = format!("phoxal/robot-api/{wire_id}");
            quote! { Self::#variant => #process_id }
        });
        let catalogue_versions = versions.iter().map(|version| {
            let variant = format_ident!("V{}", version.name.to_string().trim_start_matches('v'));
        ...
        let catalogue_names = versions.iter().map(|version| {
            let variant = format_ident!("V{}", version.name.to_string().trim_start_matches('v'));
            quote! { RobotApi::#variant }
        });
        let latest_variant = format_ident!("V{}", latest.to_string().trim_start_matches('v'));
        let catalogue = emit_catalogue.then(|| quote! {
            /// Every concrete robot API revision generated by this source
            /// tree, in declaration order.
            #[doc(hidden)]
            #[allow(non_camel_case_types)]
            #[derive(Clone, Copy, Debug, Eq, PartialEq, Ord, PartialOrd, Hash)]
            pub enum RobotApi {
                #(#catalogue_declarations),*
            }
            impl RobotApi {
                pub const fn as_str(self) -> &'static str {
                    match self { #(#catalogue_variants),* }
                }
                pub const fn version(self) -> ::phoxal_runtime_contract::version::RobotApiVersion {
                    match self { #(#catalogue_versions),* }
                }
                ...
                pub const ALL: &'static [Self] = &[#(#catalogue_names),*];
...
mod tests {
    ...
    #[test]
    fn latest_is_attached_to_the_final_revision_and_emits_catalogue() {
        let expanded = compact_tokens(
            expand_api(quote! {
                latest version v0.2 {
                    sample { topic body: State<crate::Payload>; }
                }
            })
            .expect("attached latest expands"),
        );
