//! The official navigation Runtime.
//!
//! Navigation owns goal admission, a bounded search phase, following, and
//! terminal goal history.  Transport, map acquisition, and localization are
//! inputs to the Runtime; none of those concerns mutate the planner from a
//! background task.
...
fn validate_navigation_config(config: &NavigationConfig) -> phoxal::Result<()> {
